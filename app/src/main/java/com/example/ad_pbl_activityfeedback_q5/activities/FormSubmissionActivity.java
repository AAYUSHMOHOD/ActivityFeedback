...
public class FormSubmissionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FirebaseHelper.hasStudentSubmittedForm(formId, userId, hasSubmitted -> {
            if (hasSubmitted) {
                // Student has already submitted, show toast and redirect
                Toast.makeText(this, "You have already submitted this form", Toast.LENGTH_SHORT).show();
                ...
                FirebaseHelper.getStudentSubmission(formId, userId, submission -> {
                    if (submission != null) {
                        ...
                        Toast.makeText(this, "Error retrieving your submission", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                });
            } else {
                // Continue with form loading
                loadForm();
    ...
    private void addQuestionView(Question question) {
        ...
        questionViews.put(question.getQuestionId(), questionView);
    ...
    private void submitForm() {
        ...
        // Get current user with roll number
        FirebaseHelper.getCurrentUser(user -> {
            if (user != null && user.getRollNumber() != null) {
                String rollNumber = user.getRollNumber();

                // Create submission with roll number
                Submission submission = new Submission(null, formId, userId, rollNumber);
                ...
                progressBar.setVisibility(View.GONE);
                submitButton.setEnabled(true);
                Toast.makeText(this, "Error: Could not retrieve your roll number", Toast.LENGTH_SHORT).show();
            }
        });
    }
